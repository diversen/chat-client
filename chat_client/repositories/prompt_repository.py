...
async def _validate(title: str, prompt_text: str):
    if not title:
        raise UserValidate("Title is required")
    if len(title) > MAX_TITLE_LEN:
        raise UserValidate(f"Title must be ≤ {MAX_TITLE_LEN} characters")
    if not prompt_text:
        raise UserValidate("Prompt is required")
    if len(prompt_text) > MAX_PROMPT_LEN:
        raise UserValidate(f"Prompt must be ≤ {MAX_PROMPT_LEN} characters")


async def create_prompt(request: Request):
    user_id = await user_session.is_logged_in(request)
    if not user_id:
        raise UserValidate("You must be logged in to create prompts")
...
async def update_prompt(request: Request, prompt_id: int):
    user_id = await user_session.is_logged_in(request)
    if not user_id:
        raise UserValidate("You must be logged in to update prompts")
    ...
    async with async_session() as session:
        ...
        if result.rowcount == 0:
            raise UserValidate("Prompt not found or no permission")
        await session.commit()
...
async def delete_prompt(request: Request, prompt_id: int):
    user_id = await user_session.is_logged_in(request)
    if not user_id:
        raise UserValidate("You must be logged in to delete prompts")
