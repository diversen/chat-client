        ...
        with TestClient(self.app) as client:
            ...
            with patch('chat_client.core.user_session.is_logged_in', return_value=1):
                ...
                with patch('chat_client.repositories.chat_repository.create_message', return_value=456):
                    response = client.post("/chat/create-message/dialog-123", json={
                ...
                with patch('chat_client.repositories.chat_repository.get_messages', return_value=[
                    {"message_id": 456, "content": "Test message", "role": "user"}
                ]):
                ...
                # Get dialog
                with patch('chat_client.repositories.chat_repository.get_dialog', return_value={
                    "dialog_id": "dialog-123", "title": "Test Dialog"
                }):
    ...
    def test_user_endpoints(self):
        ...
        with TestClient(self.app) as client:
            ...
            with patch('chat_client.repositories.user_repository.new_password', return_value=True):
                response = client.post("/user/new-password", json={
                    "token": "reset-token",
            ...
            with patch('chat_client.core.user_session.is_logged_in', return_value=1):
                ...
                with patch('chat_client.repositories.user_repository.get_profile', return_value={
                    "email": "test@example.com", "system_message": "Test system message"
                }):
                ...
                # List dialogs
                with patch('chat_client.repositories.chat_repository.get_dialogs_info', return_value=[
        ...
        with TestClient(self.app) as client:
            ...
            with patch('chat_client.core.user_session.is_logged_in', return_value=1):
                ...
                with patch('chat_client.repositories.prompt_repository.create_prompt', return_value={"prompt_id": 2}):
                    response = client.post("/prompt/create", json={
                        "title": "New Prompt",
                ...
                with patch('chat_client.repositories.prompt_repository.get_prompt', return_value=mock_prompt):
                    with patch('chat_client.repositories.prompt_repository.update_prompt', return_value=True):
                        response = client.post("/prompt/1/edit", json={
    ...
    def test_error_endpoints(self):
        ...
        with TestClient(self.app) as client:
            ...
            with patch('chat_client.endpoints.error_endpoints.log') as mock_logger:
                response = client.post("/error/log", json={
            ...
            for method, url, *json_data in protected_routes:
                ...
                with patch('chat_client.core.user_session.is_logged_in', return_value=False):
                    try:
                        ...
                        assert response.status_code in [307, 401], f"Route {method} {url} returned {response.status_code}, expected 307 or 401"
